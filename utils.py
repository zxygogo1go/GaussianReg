...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, size, mode='bilinear', device=None):
        ...
        # ``persistent=False`` keeps legacy checkpoint grids from causing shape
        # mismatches when a checkpoint is loaded for a different input shape.
        self.register_buffer('grid', grid, persistent=False)

    def forward(self, src, flow):
        # new locations
        grid = self.grid.to(device=flow.device, dtype=flow.dtype)
        new_locs = grid + flow
        shape = flow.shape[2:]

        # need to normalize grid values to [-1, 1] for resampler
        normalizer = flow.new_tensor([max(int(s) - 1, 1) for s in shape]).view(
            1, len(shape), *([1] * len(shape))
        )
        new_locs = 2.0 * (new_locs / normalizer - 0.5)
