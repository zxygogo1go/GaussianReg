...
class GaussianTokenRegularization(nn.Module):
    """Coverage, diversity, compactness, type, and visibility priors."""

    def __init__(self, repulsion_scale=0.08):
        super().__init__()
        self.repulsion_scale = float(repulsion_scale)

    def forward(self, tokens):
        if tokens.attention is None:
            raise ValueError('Gaussian token regularization requires stored attention')
        attention = tokens.attention.float()
        num_tokens = attention.shape[1]
        num_voxels = attention.shape[2]
        coverage = ((attention.sum(dim=1) - float(num_tokens) / float(num_voxels)) ** 2).mean()

        pair_distance = torch.cdist(tokens.mu.float(), tokens.mu.float()).square()
        if num_tokens > 1:
            off_diagonal = ~torch.eye(num_tokens, device=pair_distance.device, dtype=torch.bool)
            repulsion = torch.exp(-pair_distance / self.repulsion_scale)[:, off_diagonal].mean()
        else:
            repulsion = pair_distance.new_zeros(())
        compact = tokens.cov.float().diagonal(dim1=-2, dim2=-1).sum(dim=-1).mean()

        anatomy = tokens.anatomy.float().clamp_min(1.0e-8)
        per_token_entropy = -(anatomy * anatomy.log()).sum(dim=-1).mean()
        mean_anatomy = anatomy.mean(dim=1)
        population_entropy = -(mean_anatomy * mean_anatomy.log()).sum(dim=-1).mean()
        type_diversity = per_token_entropy - population_entropy
        visibility = (tokens.visibility.float().mean() - 0.9).square()
        return coverage + 0.1 * repulsion + 0.01 * compact + 0.05 * type_diversity + 0.1 * visibility


class TransportCostLoss(nn.Module):
    def forward(self, transport, cost):
        if transport.shape != cost.shape:
            raise AssertionError('transport and cost shapes must match')
        return (transport.float() * cost.float()).sum() / transport.float().sum().clamp_min(1.0e-6)


class AnchorFlowConsistencyLoss(nn.Module):
    """Confidence-weighted Charbonnier agreement at fixed token centers."""

    def __init__(self, epsilon=1.0e-6):
        super().__init__()
        self.epsilon = float(epsilon)

    def forward(self, flow, fixed_mu, anchor_disp, anchor_conf):
        if flow.ndim != 5 or flow.shape[1] != 3:
            raise AssertionError('flow must have shape [B,3,D,H,W]')
        if fixed_mu.shape != anchor_disp.shape or fixed_mu.shape[-1] != 3:
            raise AssertionError('fixed_mu and anchor_disp must have shape [B,N,3]')
        if anchor_conf.shape != fixed_mu.shape[:2] + (1,):
            raise AssertionError('anchor_conf must have shape [B,N,1]')
        grid = fixed_mu[..., [2, 1, 0]].unsqueeze(2).unsqueeze(2)
        ...
        error = torch.sqrt((sampled - anchor_disp).float().square() + self.epsilon).mean(dim=-1)
        confidence = anchor_conf.float().squeeze(-1)
        return (error * confidence).sum() / confidence.sum().clamp_min(1.0e-6)
